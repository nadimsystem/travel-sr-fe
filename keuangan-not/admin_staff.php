...
<body class="h-screen w-screen bg-slate-50 text-slate-800 dark:bg-slate-900 dark:text-slate-100 overflow-hidden">
    <div id="app" class="flex h-full w-full" v-cloak>
        <!-- Sidebar -->
        <?php $currentPage = 'admin_staff'; include 'components/sidebar.php'; ?>
                ...
                <div class="bg-white dark:bg-slate-800 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-fade-in">
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <!-- Password Hidden as requested -->
                         <div><label class="text-xs font-bold text-slate-500 uppercase mb-1 block">Password {{ modalMode == 'edit' ? '(Kosongkan jika tidak ubah)' : '' }}</label><input type="password" v-model="form.password" class="w-full p-2.5 border rounded-lg dark:bg-slate-700 dark:border-slate-600 dark:text-white text-sm" placeholder="******"></div>
