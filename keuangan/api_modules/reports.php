...
if ($action === 'get_reports') {
    ...
    $detailSql = "SELECT date, time, REPLACE(routeName, ' (Normal)', '') as routeName, COUNT(id) as count, SUM(seatCount) as seats, SUM(totalPrice) as tripRevenue 
                  FROM bookings 
                  WHERE status NOT IN ('Cancelled', 'Batal') $routeCondition AND (DATE_FORMAT(date, '%Y-%m') = '$monthFilter' OR date IN ('" . implode("','", $labels) . "'))
                  GROUP BY date, time, REPLACE(routeName, ' (Normal)', '')";
...
if ($action === 'get_dashboard_summary') {
    try {
        ...
        // 1. TODAY'S STATS (Fast Aggregation)
        ...
        $sqlToday = "SELECT 
            ...
            FROM bookings 
            WHERE status NOT IN ('Cancelled', 'Batal') AND date = '$today'";
        ...
        $sqlMonth = "SELECT 
            ...
            FROM bookings 
            WHERE status NOT IN ('Cancelled', 'Batal') AND date BETWEEN '$startDate' AND '$endDate'";
        $resMonth = $conn->query($sqlMonth)->fetch_assoc();

        // 3. PENDING DISPATCH (Simple Count)
        ...
        $sqlGraph = "SELECT date, SUM(totalPrice) as revenue, SUM(seatCount) as pax
                     FROM bookings
                     WHERE status NOT IN ('Cancelled', 'Batal')
                     AND date BETWEEN DATE_SUB('$today', INTERVAL 30 DAY) AND '$today'
                     GROUP BY date
                     ORDER BY date ASC";
        ...
        while($r = $resGraph->fetch_assoc()) {
            $graphLabels[] = $r['date'];
            $graphRevenue[] = (float)$r['revenue'];
            $graphPax[] = (int)$r['pax'];
        }

        echo json_encode(['status' => 'success', 'data' => [
            'today' => [
                'revenue' => (float)($resToday['revenue'] ?? 0),
                'pax' => (int)($resToday['pax'] ?? 0), 
                'pendingValidation' => (int)($resToday['pendingValidation'] ?? 0)
            ],
            'month' => [
                'revenue' => (float)($resMonth['revenue'] ?? 0),
                'pax' => (int)($resMonth['paxTotal'] ?? 0)
            ],
