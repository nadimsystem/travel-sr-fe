...
createApp({
    data() {
        return {
            ...
            historySearchTerm: '',
            now: new Date(),
            currentTime: this.formatTime(new Date(), true),
            ...
            return this.trips.map(trip => {
                ...
                if (trip.status === 'Tiba') {
                    if (trip.type === 'Rental') {
                        ...
                        if(paxString.toLowerCase() === 'full'){
                            numberOfPassengers = fleet.capacity || 0;
                        } else {
                            const paxMatch = paxString.match(/^(\d+)/);
                            numberOfPassengers = paxMatch ? parseInt(paxMatch[1], 10) : 0;
                        }
    ...
    methods: {
        ...
        formatTime(date, withSeconds = false) { const d = new Date(date); if (isNaN(d)) return '--:--'; const options = { hour: '2-digit', minute: '2-digit' }; if (withSeconds) options.second = '2-digit'; return d.toLocaleTimeString('id-ID', options); },
