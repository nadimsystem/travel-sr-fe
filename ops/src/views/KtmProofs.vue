<template>
  <div class="flex flex-col lg:flex-row gap-4 lg:gap-6 h-[calc(100vh-8rem)]">
    ...
    <div class="w-full lg:w-64 bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden flex flex-col flex-shrink-0 h-auto max-h-48 lg:max-h-full lg:h-auto transition-all">
        <div class="p-3 lg:p-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 flex justify-between items-center cursor-pointer lg:cursor-default" @click="toggleMobileDates">
            ...
            <div class="flex flex-col xl:flex-row justify-between xl:items-center gap-4">
                 <div>
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h2 class="font-bold text-lg text-slate-800 dark:text-white leading-tight">Bukti KTM</h2>
        ...
        <div class="flex-1 overflow-y-auto p-4 lg:p-6 bg-slate-50/50 dark:bg-slate-900/50 relative">
             ...
             <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 lg:gap-6">
                 <div v-for="item in filteredProofs" :key="item.id" class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group">
                      <!-- Image Header -->
                      <div class="h-56 bg-slate-200 dark:bg-slate-700 relative overflow-hidden cursor-pointer" @click="viewImage(getImageUrl(item.ktmProof))">
                          <img :src="getImageUrl(item.ktmProof)" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" loading="lazy">
                      ...
                      <div class="p-5">
                          ...
                          <div class="flex gap-2">
                              <a :href="'https://wa.me/' + formatPhone(item.passengerPhone)" target="_blank" class="flex-1 bg-green-50 text-green-600 hover:bg-green-100 border border-green-200 py-2.5 rounded-lg flex items-center justify-center gap-2 transition-colors font-bold text-xs">
...
const fetchDates = async () => {
    ...
    try {
        const res = await axios.get('api.php?action=get_ktm_proofs');
...
const fetchProofs = async (date) => {
    ...
    proofs.value = []; // Clear current proofs to avoid confusion
    try {
        const res = await axios.get(`api.php?action=get_ktm_proofs&date=${date}`);
        if (res.data.status === 'success') {
            proofs.value = res.data.proofs;
        }
    } catch (e) {
        console.error(e);
        Swal.fire('Error', 'Gagal memuat bukti KTM', 'error');
    } finally {
