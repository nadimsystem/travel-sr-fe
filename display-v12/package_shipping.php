    ...
    <div id="app" class="flex h-full w-full" v-cloak>
        ...
        <main class="flex-1 flex flex-col min-w-0 bg-slate-50 dark:bg-slate-900 relative h-full overflow-hidden transition-colors duration-300">
                                    ...
                                    <div class="space-y-4">
                                        ...
                                        <div>
                                            ...
                                            <select v-model="packageForm.route" @change="calculatePackagePrice" class="w-full p-2.5 border rounded-xl dark:bg-slate-700 dark:border-slate-600 text-sm font-bold">
                                                ...
                                                <option>Bukittinggi - Padang</option>
                                                <option>Padang - Payakumbuh</option>
                                                <option>Payakumbuh - Padang</option>
                                                <option>Bukittinggi - Payakumbuh</option>
                                                <option>Payakumbuh - Bukittinggi</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="bg-white dark:bg-slate-800 w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                    ...
                    <div class="p-6 overflow-y-auto custom-scrollbar flex-1">
                        ...
                        <div class="relative space-y-0">
                             <div v-for="(log, idx) in activePackageLogs" :key="idx" class="flex gap-4 group">
                                 <!-- Left: Time -->
                                 <div class="w-16 flex-shrink-0 text-right pt-0.5">
                                     <div class="text-[10px] font-bold text-slate-400">{{ formatDateComplete(log.created_at).split(' ')[0] }}</div>
                                     <div class="text-xs font-bold text-slate-800 dark:text-white">{{ formatDateComplete(log.created_at).split(' ')[1] }}</div>
                                 </div>

                                 <!-- Center: Line/Dot -->
                                 <div class="relative flex flex-col items-center">
                                     <div class="w-3 h-3 rounded-full border-2 border-white dark:border-slate-800 z-10" 
                                          :class="idx === 0 ? 'bg-blue-600 ring-4 ring-blue-100 dark:ring-blue-900/30 w-4 h-4' : 'bg-slate-300 dark:bg-slate-600'"></div>
                                     <div v-if="idx !== activePackageLogs.length - 1" class="flex-1 w-0.5 bg-slate-200 dark:bg-slate-700 my-1"></div>
                                 </div>

                                 <!-- Right: Content -->
                                 <div class="flex-1 pb-8 group-last:pb-0">
                                     <div class="bg-white dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700/50 rounded-2xl p-4 shadow-sm relative top-[-10px]">
                                         <h4 class="font-bold text-slate-800 dark:text-white flex items-center justify-between" :class="{'text-blue-600': idx === 0}">
                                             {{ log.status }}
                                             <span class="text-[9px] px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-500">{{ log.admin_name }}</span>
                                         </h4>
                                         <p class="text-sm text-slate-500 mt-1">
                                             {{ log.description }}
                                         </p>
                                         <div v-if="log.location && log.location !== '-'" class="mt-2 text-xs font-bold text-slate-500 flex items-center gap-1.5 bg-slate-50 dark:bg-slate-900 w-fit px-2 py-1 rounded-lg">
                                             <i class="bi bi-geo-alt-fill text-blue-500"></i> {{ log.location }}
                                         </div>
                                     </div>
                                 </div>
                             </div>
                             <div v-if="activePackageLogs.length === 0" class="text-center py-8 text-sm text-slate-400 italic">Belum ada riwayat update.</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div v-if="isStatusModalVisible" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-fade-in">
                <div class="bg-white dark:bg-slate-800 w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row max-h-[90vh]">
                    
                    <!-- Left: Form -->
                    <div class="flex-1 p-6 md:border-r border-slate-100 dark:border-slate-700 overflow-y-auto custom-scrollbar">
                    ...
                    <!-- Right: History Timeline (Compact) -->
                    <div class="w-full md:w-96 bg-slate-50 dark:bg-slate-900/50 p-6 overflow-y-auto custom-scrollbar">
                        <h3 class="text-sm font-bold text-slate-500 uppercase tracking-wider mb-4">Riwayat Terkini</h3>
                        
                        <div class="relative space-y-0">
                             <div v-for="(log, idx) in activePackageLogs" :key="idx" class="flex gap-3 group">
                                 <!-- Left: Time -->
                                 <div class="w-12 flex-shrink-0 text-right pt-0.5">
                                     <div class="text-[9px] font-bold text-slate-400">{{ formatDateComplete(log.created_at).split(' ')[0] }}</div>
                                     <div class="text-[10px] font-bold text-slate-800 dark:text-white">{{ formatDateComplete(log.created_at).split(' ')[1] }}</div>
                                 </div>

                                 <!-- Center: Line/Dot -->
                                 <div class="relative flex flex-col items-center">
                                     <div class="w-2.5 h-2.5 rounded-full border-2 border-white dark:border-slate-800 z-10" 
                                          :class="idx === 0 ? 'bg-blue-600 ring-2 ring-blue-100 dark:ring-blue-900/30 w-3 h-3' : 'bg-slate-300 dark:bg-slate-600'"></div>
                                     <div v-if="idx !== activePackageLogs.length - 1" class="flex-1 w-px bg-slate-200 dark:bg-slate-700 my-1"></div>
                                 </div>

                                 <!-- Right: Content -->
                                 <div class="flex-1 pb-6 group-last:pb-0">
                                     <div class="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-xl p-3 shadow-sm relative top-[-6px]">
                                         <h4 class="text-xs font-bold text-slate-800 dark:text-white flex items-center justify-between" :class="{'text-blue-600': idx === 0}">
                                             {{ log.status }}
                                         </h4>
                             ...
                             <div v-if="activePackageLogs.length === 0" class="text-center py-8 text-xs text-slate-400 italic">Belum ada riwayat update.</div>
                        </div>
                    </div>
