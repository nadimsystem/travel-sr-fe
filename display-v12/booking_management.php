        ...
        <main class="flex-1 flex flex-col min-w-0 bg-slate-50 dark:bg-slate-900 relative h-full overflow-hidden transition-colors duration-300">
            ...
            <div class="flex-1 relative overflow-hidden w-full">
                        ...
                        <table v-if="bookingManagementTab === 'travel' || busViewMode === 'list'" class="w-full text-left text-sm">
                            ...
                            <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                                <tr v-for="(b, index) in paginatedBookings" :key="b.id" @click="openDetailModal(b)" class="hover:bg-blue-50/30 dark:hover:bg-slate-700/50 transition-colors cursor-pointer group">
                                    ...
                                    <td class="p-4 max-w-[200px]">
                                        <div class="flex flex-col gap-1.5">
                                            ...
                                            <div class="mt-1 pt-1 border-t border-dashed border-slate-200">
                                                ...
                                                <span class="text-[9px] text-slate-400 ml-1">{{ (b.routeId && String(b.routeId).startsWith('CUSTOM_')) ? 'Carter Khusus' : b.routeId }}</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="p-4">
                                        <div class="flex flex-col gap-1">
                                            ...
                                            <div v-if="b.paymentMethod === 'Cash'" class="text-[9px] text-slate-600 dark:text-slate-400 space-y-0.5 mt-1">
                                                <div v-if="b.paymentReceivedDate"><i class="bi bi-calendar-check text-emerald-600"></i> {{ formatDate(b.paymentReceivedDate) }}</div>
                                            ...
                                            <div v-if="b.paymentMethod === 'Transfer'" class="text-[9px] text-slate-600 dark:text-slate-400 space-y-0.5 mt-1">
                                                <div v-if="b.transferSentDate"><i class="bi bi-calendar-check text-blue-600"></i> {{ formatDate(b.transferSentDate) }}</div>
                                                <div v-if="b.destinationAccount"><i class="bi bi-credit-card text-blue-600"></i> {{ b.destinationAccount }}</div>
                                            </div>
                                    ...
                                    <td class="p-4">
                                        <div class="text-[10px] text-slate-500 italic max-w-[150px] truncate" :title="b.bookingNote">{{ b.bookingNote || '-' }}</div>
                                    </td>
                                    <td class="p-4 text-right space-x-1" @click.stop>
                                        ...
                                        <button v-if="b.validationStatus !== 'Valid'" @click="validatePaymentModal(b)" class="p-2 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-lg transition-colors" title="Validasi Pembayaran">
                ...
                <div v-if="isValidationModalVisible" class="fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                    <div class="bg-white p-6 rounded-2xl text-center max-w-4xl w-full shadow-2xl animate-fade-in">
                        ...
                        <div v-if="validationData.paymentMethod === 'Cash'" class="bg-gray-50 p-6 rounded-xl border border-gray-200 mb-6 text-left space-y-3">
                                ...
                                <div class="flex justify-between pt-1">
                                    ...
                                    <div class="text-right">
                                        <div class="font-bold text-blue-600">Rp {{ (validationData.totalPrice||0).toLocaleString('id-ID') }}</div>
                                        <div v-if="validationData.serviceType === 'Dropping' || validationData.serviceType === 'Carter'" class="text-[10px] text-gray-400">Harga Unit / Paket</div>
                                        <div v-else class="text-[10px] text-gray-400">{{ validationData.seatCount }} Orang x @ Rp {{ (validationData.seatCount > 0 ? (validationData.totalPrice / validationData.seatCount) : 0).toLocaleString('id-ID') }}</div>
                                    </div>
                                </div>
                            </div>

                        <div v-if="(validationData.paymentMethod === 'Transfer' || validationData.paymentMethod === 'DP') && validationData.paymentProof" class="mb-6">
                            <div class="bg-slate-100 p-2 rounded-xl mb-3 border border-slate-200">
                                <!-- Full image preview (uncropped) -->
                                <img :src="validationData.paymentProof" class="w-full h-auto max-h-[60vh] object-contain rounded-lg shadow-sm bg-white">
                            </div>
                            
                            <!-- Dedicated Zoom/Open Button -->
                            <button @click="window.open(validationData.paymentProof, '_blank')" class="w-full py-2 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-lg text-xs font-bold transition-colors flex items-center justify-center gap-2 border border-blue-200 mb-4">
                                <i class="bi bi-arrows-fullscreen"></i> Perbesar / Download Bukti
                            </button>
                        ...
                        <div class="flex gap-3">
                            ...
                            <button @click="confirmValidation('Valid')" class="flex-1 py-3 rounded-xl bg-green-600 text-white font-bold hover:bg-green-700 shadow-lg shadow-green-200 transition-transform active:scale-95 flex items-center justify-center gap-2"><i class="bi bi-check-lg"></i> Validasi Lunas</button>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="bg-white dark:bg-slate-800 w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-fade-in">
                         ...
                         <div class="p-6 overflow-y-auto custom-scrollbar space-y-6">
                                 ...
                                 <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
                                     <div class="p-4 flex justify-between items-center bg-slate-50 dark:bg-slate-700/30 border-b border-slate-100 dark:border-slate-700">
                                         <div>
                                             ...
                                             <div class="text-xl font-bold text-blue-600">Rp {{ (detailModalData.totalPrice||0).toLocaleString('id-ID') }}</div>
                                             <div v-if="detailModalData.serviceType === 'Dropping' || detailModalData.serviceType === 'Carter'" class="text-[10px] text-slate-400 mt-0.5 font-medium">Harga Unit / Paket</div>
                                             <div v-else class="text-[10px] text-slate-400 mt-0.5 font-medium">{{ detailModalData.seatCount }} Penumpang x @ Rp {{ (detailModalData.seatCount > 0 ? (detailModalData.totalPrice / detailModalData.seatCount) : 0).toLocaleString('id-ID') }}</div>
                                         </div>
                                         <div class="text-right">
                                            ...
                                            <div class="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-xs font-bold uppercase" v-else-if="detailModalData.paymentMethod === 'Transfer' && detailModalData.paymentProof && detailModalData.validationStatus !== 'Valid'">Menunggu Validasi</div>
                                     ...
                                     <div v-if="detailModalData.validationStatus !== 'Valid'" class="p-3 bg-red-50 dark:bg-red-900/10 border-b border-red-100 dark:border-red-900/30 flex justify-between items-center">
                                         ...
                                         <div class="text-base font-extrabold text-red-600 dark:text-red-400">
                                             Rp {{ ((detailModalData.totalPrice - (detailModalData.downPaymentAmount || 0)) || 0).toLocaleString('id-ID') }}
                                         </div>
                                     </div>

                                     <div class="p-4 grid grid-cols-2 gap-4 text-sm bg-slate-50 dark:bg-slate-700/30 rounded-xl border border-dashed border-slate-200 dark:border-slate-600">
                                         <div>
                                             <div class="text-xs text-slate-400 uppercase font-bold mb-1">Metode Pembayaran</div>
                                             <div class="font-bold flex items-center gap-2 text-slate-700 dark:text-slate-300">
                                          ...
                                          <div>
                                             <!-- TRANSFER INFO -->
                                             <div v-if="detailModalData.paymentMethod === 'Transfer' || (detailModalData.paymentMethod === 'DP' && detailModalData.transferSentDate)">
                                                 <div class="text-xs text-slate-400 uppercase font-bold mb-1">Detail Transfer</div>
                                                 <div class="space-y-1">
                                                     <div v-if="detailModalData.transferSentDate" class="text-xs"><span class="text-slate-500">Tgl:</span> <b>{{ formatDate(detailModalData.transferSentDate) }}</b></div>
                                                     <div v-if="detailModalData.destinationAccount" class="text-xs"><span class="text-slate-500">Ke:</span> <b>{{ detailModalData.destinationAccount }}</b></div>
                                                     <div v-if="detailModalData.paymentProof">
                                                         <a :href="detailModalData.paymentProof" target="_blank" class="text-blue-600 hover:underline flex items-center gap-1 text-xs font-bold">
                                             ...
                                             <!-- CASH INFO -->
                                             <div v-else-if="detailModalData.paymentMethod === 'Cash' || (detailModalData.paymentMethod === 'DP' && detailModalData.paymentReceivedDate)">
                                                  <div class="text-xs text-slate-400 uppercase font-bold mb-1">Detail Cash</div>
                                                  <div class="space-y-1">
                                                     <div v-if="detailModalData.paymentReceivedDate" class="text-xs"><span class="text-slate-500">Tgl:</span> <b>{{ formatDate(detailModalData.paymentReceivedDate) }}</b></div>
                                                     <div class="text-xs"><span class="text-slate-500">Lokasi:</span> <b>{{ detailModalData.paymentLocation || '-' }}</b></div>
                                                     <div class="text-xs"><span class="text-slate-500">Penerima:</span> <b>{{ detailModalData.paymentReceiver || '-' }}</b></div>
                                                  </div>
                                             </div>

                                             <!-- OTHER (DP without specific date info yet / Fallback) -->
                                             <div v-else>
                                                 <div class="text-xs text-slate-400 uppercase font-bold mb-1">
                                                     {{ detailModalData.paymentProof ? 'Bukti Transfer' : 'Lokasi / Penerima' }}
                                                 </div>
                                                 <div v-if="detailModalData.paymentProof" class="font-bold">
                                                     <a :href="detailModalData.paymentProof" target="_blank" class="text-blue-600 hover:underline flex items-center gap-1">
                                                         <i class="bi bi-image"></i> Lihat Bukti
                                                         <span v-if="detailModalData.transferSentDate">({{ formatDate(detailModalData.transferSentDate) }})</span>
                                                     </a>
                                                 </div>
                                                 <div v-else class="font-bold text-slate-700 dark:text-slate-300">
                                                     ...
                                                     <div v-if="detailModalData.paymentReceivedDate" class="text-[10px] text-slate-500 font-normal">Tgl: {{ formatDate(detailModalData.paymentReceivedDate) }}</div>
                                                 </div>
                                             </div>
                                          </div>
                                     </div>
                             ...
                             <div class="border-t border-slate-100 dark:border-slate-700 pt-6">
                                 ...
                                 <!-- Add Payment Button -->
                                 <!-- Action Button: Validasi vs Tambah Pembayaran -->
                                 <div v-if="detailModalData.validationStatus !== 'Valid'" class="mt-4">
                                     <button v-if="detailModalData.paymentMethod === 'Transfer' && detailModalData.paymentProof" @click="validatePaymentModal(detailModalData)" class="w-full px-4 py-3 bg-gradient-to-r from-green-600 to-green-700 text-white font-bold rounded-xl hover:from-green-700 hover:to-green-800 transition-all shadow-lg flex items-center justify-center gap-2">
                                         <i class="bi bi-check-circle-fill"></i> Validasi Pembayaran
                                     </button>
                                     <button v-else @click="openAddPaymentModal(detailModalData)" class="w-full px-4 py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white font-bold rounded-xl hover:from-blue-700 hover:to-blue-800 transition-all shadow-lg flex items-center justify-center gap-2">
                        ...
                        <div class="p-6 space-y-4">
                            <div class="bg-slate-50 dark:bg-slate-700/50 p-4 rounded-xl space-y-2">
                                <div class="flex justify-between text-sm">
                                    ...
                                    <span class="font-bold">Rp {{ (activePaymentBooking?.totalPrice || 0).toLocaleString('id-ID') }}</span>
                                </div>
                                <div class="flex justify-between text-sm">
                                ...
                                <div class="flex justify-between text-sm pt-2 border-t border-slate-200 dark:border-slate-600">
                                    ...
                                    <span class="font-bold text-red-600">Rp {{ ((activePaymentBooking?.totalPrice - (activePaymentBooking?.downPaymentAmount || 0)) || 0).toLocaleString('id-ID') }}</span>
                                </div>
                            </div>
                            ...
                            <div v-if="addPaymentForm.payment_method === 'Transfer'" class="space-y-4">
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <label class="text-xs font-bold text-slate-500 uppercase block mb-2">Nama Pengirim</label>
                                        <input type="text" v-model="addPaymentForm.payment_receiver" class="w-full p-3 border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 rounded-xl text-sm outline-none" placeholder="a.n. Siapa?">
                                    </div>
                                    <div>
                                        <label class="text-xs font-bold text-slate-500 uppercase block mb-2">Bank Tujuan</label>
                                        <input type="text" v-model="addPaymentForm.payment_location" class="w-full p-3 border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 rounded-xl text-sm outline-none" placeholder="ke BRI/BCA/Dana?">
                                    </div>
                                </div>
                                <div>
                                ...
                                <div v-if="addPaymentForm.payment_proof" class="relative w-fit">
                                    <img :src="addPaymentForm.payment_proof" class="h-24 rounded-lg border">
                                    <button @click="addPaymentForm.payment_proof=''" class="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs"><i class="bi bi-x"></i></button>
                                </div>
                            </div>
