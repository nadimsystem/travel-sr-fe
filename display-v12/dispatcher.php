...
<head>
    ...
    <script>
        window.onerror = function(message, source, lineno, colno, error) {
            ...
            errDiv.innerText = 'JS Error: ' + message + ' at ' + source + ':' + lineno;
        ...
        window.addEventListener('unhandledrejection', function(event) {
            ...
            errDiv.style.padding = '10px';
            errDiv.style.zIndex = '99999';
            errDiv.innerText = 'Promise Error: ' + event.reason;
            document.body.appendChild(errDiv);
        });
    </script>
...
<body class="h-screen w-screen bg-slate-50 text-slate-800 dark:bg-slate-900 dark:text-slate-100">
    ...
    <div id="app" class="flex h-full w-full" v-cloak>
        ...
        <!-- Universal Sidebar (Mobile Only) -->
        <div class="lg:hidden">
            <?php include 'components/sidebar.php'; ?>
        </div>

        <!-- Dynamic Sidebar specific for Dispatcher (Desktop Only) -->
        <aside id="dispatcherDesktopSidebar" class="hidden lg:flex flex-col w-64 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 h-full flex-shrink-0 z-50 overflow-hidden">
            <!-- Logo Area -->
        ...
        <main class="flex-1 flex flex-col min-w-0 bg-slate-50 dark:bg-slate-900 relative h-full overflow-hidden transition-colors duration-300">
            <header class="h-16 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between px-6 shadow-sm z-10 flex-shrink-0 transition-colors duration-300">
                <div class="flex items-center gap-3">
                    <button onclick="toggleSidebar()" class="lg:hidden p-2 text-slate-500 hover:text-blue-600 transition-colors rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700">
            ...
            <div class="flex-1 relative overflow-hidden w-full">
                ...
                <div v-if="view === 'dispatcher'" class="absolute inset-0 px-6 pb-6 pt-0 md:p-6 custom-scrollbar space-y-8 overflow-y-auto" id="dispatcherContainer">
                    
                    <!-- Mobile Sticky Header -->
                    <div class="sticky top-0 z-40 -mx-6 px-6 py-4 bg-slate-50/95 backdrop-blur-md border-b border-gray-200 shadow-sm mb-6 space-y-3 md:hidden">
                         <!-- Route Selector -->
                         <div class="relative">
                            <select v-model="dispatcherRouteFilter" class="w-full appearance-none bg-white border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded-xl leading-tight focus:outline-none focus:bg-white focus:border-gray-500 font-bold text-sm shadow-sm">
                                <option value="All">Semua Rute</option>
                                <option v-for="r in dispatcherRoutesList" :value="r">{{ r }}</option>
                            </select>
                         ...
                         <!-- Time Shortcuts (Only if route selected) -->
                         <div v-if="dispatcherRouteFilter !== 'All'" class="flex overflow-x-auto gap-2 pb-1 scrollbar-hide">
                            <button v-for="time in orderedTimesForSelectedRoute" 
                                    @click="scrollToTime(time)"
                                    class="flex-shrink-0 px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-xs font-bold text-gray-600 shadow-sm hover:bg-orange-50 hover:text-orange-600 hover:border-orange-200 transition-colors whitespace-nowrap">
                                {{ time }}
                            </button>
                         </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div v-for="(batches, routeName) in filteredDispatcherViews" :key="routeName" class="mb-10">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                                <div v-for="(group, idx) in batches" :key="group.key"
                                     :id="(idx === 0 || batches[idx-1].time !== group.time) ? 'group-' + routeName.replace(/\W/g,'') + '-' + group.time.replace(':','') : null"
                                    ...
                                    <div class="p-5 border-b border-gray-50 bg-gray-50/50 flex justify-between items-start relative">
                                        ...
                                        <div class="flex flex-col items-end gap-1">
                                            ...
                                            <button @click="openScheduleModal(group.routeConfig, group.time, group.date, group.assignment, group.batchNumber)" class="text-[10px] bg-slate-100 hover:bg-slate-200 text-slate-600 px-2 py-1 rounded font-bold mt-1">
                                            ...
                                            <div v-for="p in group.passengers" :key="p.id" 
                                                ...
                                                <div class="flex justify-between items-start">
                                                    <div>
                                                        ...
                                                        <div class="text-xs text-gray-400 mt-1"><i class="bi bi-whatsapp mr-1"></i> {{ maskPhone(p.passengerPhone) }}</div>
                                                    </div>
                                                    <div>
                                                ...
                                                <div class="mt-2 flex gap-2">
                                                    ...
                                                    <template v-else>
                                                        <button v-if="(p.passengerType === 'Pelajar' || p.passengerType === 'Mahasiswa / Pelajar') && p.ktmProof" @click="viewKtm(p)" class="px-2 py-0.5 bg-blue-100 text-blue-600 rounded-lg text-[10px] font-bold flex items-center gap-1"><i class="bi bi-card-image"></i> KTM</button>
                                                        <button @click="validatePaymentModal(p)" class="px-2 py-0.5 bg-red-100 text-red-700 rounded-lg text-[10px] font-bold flex items-center gap-1 hover:bg-red-200 transition-colors"><i class="bi bi-exclamation-circle-fill"></i> Cek Bukti</button>
                                                    </template>
                                                    ...
                                                    <button @click.stop="openCancellation(p.id)" class="px-2 py-0.5 bg-red-50 text-red-600 rounded-lg text-[10px] font-bold hover:bg-red-100 transition-colors" title="Batalkan Booking"><i class="bi bi-trash3-fill"></i></button>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="p-4 border-t border-gray-100 bg-gray-50/50">
                                        <div class="flex items-center justify-between">
                                            ...
                                            <button v-else 
                                                    @click="openScheduleModal(group.routeConfig, group.time, group.date, group.assignment, group.batchNumber)" 
        ...
        <div class="bg-white dark:bg-slate-800 w-full max-w-lg rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 flex flex-col max-h-[90vh]">
            ...
            <div class="p-6 overflow-y-auto custom-scrollbar space-y-6">
             ...
             <div class="p-6 border-t border-slate-100 dark:border-slate-700 bg-slate-50/50 flex gap-3 rounded-b-2xl">
                ...
                <!-- Validation Modal (Copied from booking_management.php) -->
                <div v-if="isValidationModalVisible" class="fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                    <div class="bg-white p-6 rounded-2xl text-center max-w-4xl w-full shadow-2xl animate-fade-in">
                        ...
                        <p class="text-sm text-gray-500 mb-4">Metode: <span class="font-bold text-sr-blue uppercase">{{ validationData.paymentMethod }}</span></p>
                ...
                <!-- Departure Detail Modal (Packages & Expenses) -->
                        ...
                        <div id="ticketContent" class="bg-white rounded-[1.5rem] overflow-hidden shadow-2xl relative">
                            ...
                            <div class="bg-gray-50 p-4 border-t border-gray-100 text-center">
                                ...
                                <div class="flex justify-center">
                                    <img src="qr-link.png" class="w-16 h-16 mix-blend-multiply opacity-80">
                                </div>
                            </div>
                        </div>
                ...
                <div v-if="isValidationModalVisible" class="fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                    <div class="bg-white p-6 rounded-2xl text-center max-w-lg w-full shadow-2xl animate-fade-in">
                        ...
                        <div class="flex gap-3">
                            ...
                            <!-- Modified Logic for Belum Bayar -->
                            <button v-if="validationData.paymentMethod === 'Belum Bayar'" @click="openBillingShortcut(validationData)" class="flex-1 py-3 rounded-xl bg-sr-blue text-white font-bold hover:bg-slate-800 shadow-lg transition-transform active:scale-95 flex items-center justify-center gap-2">
                                <i class="bi bi-wallet2"></i> Bayar Sekarang
                            </button>
                            <button v-else @click="confirmValidation('Valid')" class="flex-1 py-3 rounded-xl bg-green-600 text-white font-bold hover:bg-green-700 shadow-lg shadow-green-200 transition-transform active:scale-95 flex items-center justify-center gap-2">
                                <i class="bi bi-check-lg"></i> Validasi Lunas
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <!-- Add Payment Modal (Copied from Booking Management) -->
    ...
    <script>
        // Sidebar logic is now handled by components/sidebar.php for mobile
