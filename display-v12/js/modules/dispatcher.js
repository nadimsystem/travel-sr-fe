export const dispatcherMixin = {
    ...
    computed: {
        ...
        groupedBookings() {
            ...
            pending.forEach(b => {
                // Transfer Logic: Use Physical Route if present (Numpang logic)
                const effectiveRouteId = (b.physicalRouteId && b.physicalRouteId !== 'null') ? b.physicalRouteId : b.routeId;
                
                const key = `${effectiveRouteId}_${b.date}_${b.time}`;
                if (!groups[key]) {
                    let route = this.routeConfig.find(r => r.id === effectiveRouteId);
                    if (!route && (String(effectiveRouteId).startsWith('CUSTOM_'))) {
                        route = { id: effectiveRouteId, name: 'Carter Khusus', origin: b.pickupAddress || 'Custom', destination: b.dropoffAddress || 'Custom' };
                    } else if (!route) {
                         route = { id: effectiveRouteId, origin: '?', destination: '?' };
                    }
                    
                    groups[key] = {
                        key: key,
                        routeId: effectiveRouteId, // Use Effective ID for Grouping
                        date: b.date,
                        time: b.time,
                        routeConfig: route,
                        passengers: [],
        ...
        orderedTimesForSelectedRoute() {
            if (!this.dispatcherRouteFilter || this.dispatcherRouteFilter === 'All') return [];
            const batches = this.groupedDispatcherViews[this.dispatcherRouteFilter] || [];
            if (batches.length === 0) return [];
            
            // Extract unique times
            const times = [...new Set(batches.map(b => b.time))];
            
            // Sort times (Ascending for natural reading flow in horizontal scroll)
    ...
    methods: {
        ...
        openMoveModal(booking) {
            ...
            const rawSchedules = route ? (route.schedules || []) : [];
            // Normalize schedules: handle objects {time, hidden} and filter out hidden ones
        ...
        showInvalidStatusInfo(p) { this.openBookingActions(p); },

        scrollToTime(time) {
            if (!this.dispatcherRouteFilter || this.dispatcherRouteFilter === 'All') return;
            const routeKey = this.dispatcherRouteFilter.replace(/\W/g, '');
            const timeKey = time.replace(':', '');
            
            // Selector for ID starting with group-ROUTE-TIME
            // We use querySelector to find the first matching element
            const selector = `[id^="group-${routeKey}-${timeKey}"]`;
            const el = document.querySelector(selector);
            
            if (el) {
                // Scroll container is the parent with overflow-y-auto
                // We use scrollIntoView on the element
                el.scrollIntoView({ behavior: 'smooth', block: 'center' }); // center to handle sticky header obscuring logic if start
        ...
        maskPhone(phone) {
            ...
            if (str.length < 8) return phone; // Too short to mask
            // Keep first 4 and last 3, replace middle with ****
