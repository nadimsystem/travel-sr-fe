export const paymentMixin = {
    methods: {
        ...
        openBillingShortcut(booking) {
            ...
            const url = 'penagihan?search=' + encodeURIComponent(name);
        ...
        openAddPaymentModal(booking) {
            ...
            // Calculate status
            const total = parseInt(booking.totalPrice) || 0;
            const paid = parseInt(booking.downPaymentAmount) || 0;
            const remaining = total - paid;
            
            // Set Display Data (Flattened for safety)
            this.paymentDisplay = {
                name: booking.passengerName || 'Nama Tidak Ada',
            ...
            if(this.savedReceivers && this.savedReceivers.length > 0) this.addPaymentForm.payment_receiver = this.savedReceivers[0]; 
    ...
    computed: {
        activePaymentSummary() {
            const b = this.activePaymentBooking;
            if (!b) return { total: 0, paid: 0, remaining: 0, name: '' };
            const total = parseInt(b.totalPrice) || 0;
            const paid = parseInt(b.downPaymentAmount) || 0;
            return {
                total: total,
                paid: paid,
