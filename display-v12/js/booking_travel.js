...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('paymentReceivedDate').valueAsDate = new Date();
    document.getElementById('transferSentDate').valueAsDate = new Date();
    if(document.getElementById('dpReceivedDate')) document.getElementById('dpReceivedDate').valueAsDate = new Date();
    if(document.getElementById('dpTransferSentDate')) document.getElementById('dpTransferSentDate').valueAsDate = new Date();
...
let isInterRoute = false; // Tumpangkan Feature;

// Custom Route Logic
// Custom Route Logic
function handleCustomRouteToggle(e) {
    isCustomRoute = e.target.checked;
    const customSection = document.getElementById('customRouteSection');
    const serviceTabs = document.getElementById('serviceTypeTabs');
    const routeSelectDiv = document.getElementById('routeSelect')?.parentElement;
    ...
    const pickupContainer = document.getElementById('pickupAddress')?.parentElement;
    const dropoffContainer = document.getElementById('dropoffAddress')?.parentElement;
    
    // Toggle Visibility
    if (isCustomRoute) {
        customSection.classList.remove('hidden');
        if(routeSelectDiv) routeSelectDiv.classList.add('hidden');
        
        document.getElementById('passengerCategorySection')?.classList.add('hidden');
        document.getElementById('seatSelectionArea')?.classList.add('hidden');
        document.getElementById('batchSelectorArea')?.classList.add('hidden');
        
        // Hide standard Duration & Seat Count (since Custom Route has its own)
        // IF Dropping -> Hide Custom Duration as well (fixed to 1)
        if (currentServiceType === 'Dropping') {
             durationInput.classList.add('hidden'); // Original duration in form
             document.getElementById('customDuration').parentElement.classList.add('hidden'); // Custom Duration input
        } else {
             // Carter
             if(durationInput) durationInput.classList.add('hidden'); // Hide original, show custom
             document.getElementById('customDuration').parentElement.classList.remove('hidden'); 
        }

        if(seatCountInput) seatCountInput.classList.add('hidden');
        
        // Hide Standard Addresses (Replaced by Origin/Dest)
        if(pickupContainer) pickupContainer.classList.add('hidden');
        if(dropoffContainer) dropoffContainer.classList.add('hidden');
        ...
        if(routeSelectDiv) {
            ...
            // If we are still in Carter mode (likely), ensure relevant elements are shown/hidden per Carter logic
            setServiceType(currentServiceType); 
        }
        
        // Restore Standard Addresses
        if(pickupContainer) pickupContainer.classList.remove('hidden');
        if(dropoffContainer) dropoffContainer.classList.remove('hidden');
    }
    ...
    if (priceLabel) {
        if (currentServiceType === 'Dropping') priceLabel.textContent = "HARGA TOTAL";
        else priceLabel.textContent = "HARGA / HARI";
    }

    calculatePrice();
}

// Update price when custom inputs change
function handleCustomInputChange() {
    if (isCustomRoute) calculatePrice();
}
...
function updateBankAccountDisplay(val) {
    const el = document.getElementById('bankAccountDisplay');
    if(el && ACCOUNTS[val]) el.textContent = ACCOUNTS[val];
}

function updateDpBankAccountDisplay(val) {
    const el = document.getElementById('dpBankAccountDisplay');
    if(el && ACCOUNTS[val]) el.textContent = ACCOUNTS[val];
}
...
async function fetchOccupiedSeats() {
    ...
    if (isInterRoute) {
        const physicalId = document.getElementById('physicalRouteSelect').value;
        if (physicalId) routeId = physicalId;
    }
...
function setupEventListeners() {
    ...
    document.querySelectorAll('.service-type-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            // Custom Route Visibility Explicit Logic
            const customContainer = document.getElementById('customRouteContainer');
            if (customContainer) {
                if (currentServiceType === 'Carter' || currentServiceType === 'Dropping') {
                    customContainer.classList.remove('hidden');
                } else {
                    customContainer.classList.add('hidden');
                    // Reset if switching away
                    if (isCustomRoute) {
                         const toggle = document.getElementById('customRouteToggle');
                         if(toggle) {
    ...
    if (routeSelect) {
        routeSelect.addEventListener('change', () => {
            // Show Inter-Route option when main route is selected
            const interRouteSection = document.getElementById('interRouteSection');
            if(interRouteSection) interRouteSection.classList.remove('hidden');
    ...
    document.querySelectorAll('input[name="paymentMethod"]').forEach(radio => {
        radio.addEventListener('change', (e) => {
            ...
            if (e.target.value === 'Cash' && cashDiv) cashDiv.classList.remove('hidden');
                ...
                // No additional fields for Pay Later
            }
        });
    });

    // DP Method (Sub-options)
    ...
    // Custom Route Listeners
    const customToggle = document.getElementById('customRouteToggle');
    if (customToggle) customToggle.addEventListener('change', handleCustomRouteToggle);

    const customOriginSelect = document.getElementById('customOrigin');
    if (customOriginSelect) {
        customOriginSelect.addEventListener('change', (e) => {
            const manualInput = document.getElementById('customOriginManual');
    ...
    document.getElementById('customPrice')?.addEventListener('input', handleCustomInputChange);
    document.getElementById('customDuration')?.addEventListener('input', handleCustomInputChange);

    // Inter-Route Listeners
    const interRouteToggle = document.getElementById('interRouteToggle');
    if (interRouteToggle) {
        interRouteToggle.addEventListener('change', handleInterRouteToggle);
    }
    const physicalRouteSelect = document.getElementById('physicalRouteSelect');
...
function handleInterRouteToggle(e) {
    isInterRoute = e.target.checked;
    const container = document.getElementById('physicalRouteContainer');
    if (isInterRoute) {
...
function renderRouteOptions() {
    ...
    // Also populate Physical Route Select
    const physicalSelect = document.getElementById('physicalRouteSelect');
    if (physicalSelect) physicalSelect.innerHTML = '<option value="" disabled selected>Pilih Armada Tujuan...</option>';
    ...
    routes.forEach(route => {
        ...
        if (physicalSelect) {
            physicalSelect.appendChild(option.cloneNode(true));
        }
    });
}
...
function calculatePrice() {
    ...
    if (currentServiceType === 'Carter') {
        ...
        currentPrice = route.prices.carter * duration;
    } else if (currentServiceType === 'Dropping') {
        currentPrice = route.prices.dropping;
        // Optional: Add Multi-Drop logic if price increment is known
        // if (document.getElementById('multiDrop')?.checked) currentPrice += 50000; 
    } else {
        // Travel: Check Category
        const category = document.querySelector('input[name="passengerCategory"]:checked').value;
        let basePrice = 0;
        if (category === 'Pelajar') {
            basePrice = route.prices.pelajar;
        } else {
            basePrice = route.prices.umum;
        }
        
        // Multiply by selected seats
        // If no seats selected, show 0 or booking is invalid anyway
        currentPrice = basePrice * (selectedSeats.length || 0);
    }
...
async function saveBooking() {
    ...
    try {
        let routeId, route, routeName, origin, destination;
        let physicalRouteId = null;
        
        if (isCustomRoute) {

            origin = document.getElementById('customOrigin').value;
            if (origin === 'manual') {
                origin = document.getElementById('customOriginManual').value;
            }
            destination = document.getElementById('customDestination').value;

            const price = parseFloat(document.getElementById('customPrice').value);
            
            if (!origin) return alert("Pilih Titik Awal!");
            if (!destination) return alert("Isi Titik Akhir!");
            if (!price) return alert("Isi Harga!");
            
            routeId = 'CUSTOM_' + Date.now(); // Generate dummy ID
            routeName = `${origin} - ${destination}`;
            
            // Custom Route is essentially a Charter/Carter BUT we preserve Dropping if selected
            // currentServiceType = 'Carter'; // OLD
            if (currentServiceType !== 'Dropping') currentServiceType = 'Carter';
        } else {
            // Normal or Inter-Route
            let mainRouteId = document.getElementById('routeSelect').value;
            if (!mainRouteId) return alert("Pilih Rute Utama!");

            if (isInterRoute) {
                physicalRouteId = document.getElementById('physicalRouteSelect').value;
                if (!physicalRouteId) return alert("Pilih Armada Fisik (Tumpangkan)!");
                // For logic, we keep routeId as the FINANCIAL/INITIAL route
                routeId = mainRouteId; 
            } else {
                routeId = mainRouteId;
            }

            // Route Object for Name derivation (Use Initial Route)
        ...
        // Only validate passenger category if NOT custom route AND is Travel
        if (!isCustomRoute && currentServiceType === 'Travel') {
        ...
        if (passengerType === 'Pelajar' && !ktmBase64 && !isCustomRoute) {
        ...
        if (paymentMethod === 'Cash') {
            ...
            if (!document.getElementById('destinationAccount').value) return alert("Pilih Rekening Tujuan!");
            ...
            if (dpMethod === 'Cash') {
                ...
                // DP Transfer - Validate rekening tujuan FIRST before proof
                if (!document.getElementById('dpDestinationAccount').value) return alert("Pilih Rekening Tujuan!");
        ...
        // Determine Seat & Duration
        let manualSeatCount = 1;
        let duration = 1;
        
        if (isCustomRoute) {
             duration = parseInt(document.getElementById('customDuration').value) || 1;
             // For custom route, seat count is not user-selectable, assume 1 unit charter
             manualSeatCount = 1;
        } else {
             manualSeatCount = parseInt(document.getElementById('seatCountInput').value) || 1;
             duration = parseInt(document.getElementById('durationInput').value) || 1;
        }

        // FORCE 8 SEATS FOR DROPPING / CARTER
        if (currentServiceType === 'Dropping' || currentServiceType === 'Carter') {
            manualSeatCount = 8;
        }
        ...
        const bookingData = {
            ...
            time: document.getElementById('timeInput').value || '08:00',
            passengerName: document.getElementById('passengerName').value,
            passengerPhone: document.getElementById('passengerPhone').value,
            passengerType: passengerType,
            seatCount: (!isCustomRoute && currentServiceType === 'Travel') ? selectedSeats.length : manualSeatCount,
            selectedSeats: (!isCustomRoute && currentServiceType === 'Travel') ? selectedSeats : [],
            duration: duration,
            ...
            seatNumbers: (!isCustomRoute && currentServiceType === 'Travel') ? selectedSeats.join(', ') : 'Full Unit',
            ktmProof: ktmBase64,
            ...
            packageType: 'Unit',
            routeName: routeName,
            pickupAddress: isCustomRoute ? origin : (document.getElementById('pickupAddress').value + (document.getElementById('pickupMapLink').value ? ', ' + document.getElementById('pickupMapLink').value : '')),

            dropoffAddress: (isCustomRoute ? destination : (document.getElementById('dropoffAddress').value + (document.getElementById('dropoffMapLink').value ? ', ' + document.getElementById('dropoffMapLink').value : ''))) + (isInterRoute && physicalRouteId ? ` (TUMPANGAN KE: ${document.getElementById('physicalRouteSelect').options[document.getElementById('physicalRouteSelect').selectedIndex].text})` : ''),
            ...
            // New Fields
            // New Fields Logic
            paymentReceivedDate: (paymentMethod === 'Cash') 
                ? document.getElementById('paymentReceivedDate').value 
                : ((paymentMethod === 'DP' && document.querySelector('input[name="dpMethod"]:checked')?.value === 'Cash') 
                    ? document.getElementById('dpReceivedDate').value 
                    : null),

            transferSentDate: (paymentMethod === 'Transfer') 
                ? document.getElementById('transferSentDate').value 
                : ((paymentMethod === 'DP' && document.querySelector('input[name="dpMethod"]:checked')?.value === 'Transfer') 
                    ? document.getElementById('dpTransferSentDate').value 
                    : null),

            destinationAccount: (paymentMethod === 'Transfer') 
                ? document.getElementById('destinationAccount').value 
                : ((paymentMethod === 'DP' && document.querySelector('input[name="dpMethod"]:checked')?.value === 'Transfer') 
                    ? document.getElementById('dpDestinationAccount').value 
                    : null),

            // Send admin name to trigger 'Pending' status (Accepted)
            adminName: window.currentUserName || ''
        };
