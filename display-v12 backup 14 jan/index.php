...
<body class="h-screen w-screen bg-slate-50 text-slate-800 dark:bg-slate-900 dark:text-slate-100">
    ...
    <div id="app" class="flex h-full w-full" v-cloak>
        ...
        <!-- Loading Overlay -->
        <div v-if="isLoading" class="fixed inset-0 z-[60] flex flex-col items-center justify-center bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm animate-fade-in">
            <div class="relative w-20 h-20 mb-4">
               <div class="absolute inset-0 border-4 border-slate-200 dark:border-slate-700 rounded-full"></div>
               <div class="absolute inset-0 border-4 border-sr-blue dark:border-blue-500 rounded-full border-t-transparent animate-spin"></div>
               <div class="absolute inset-0 flex items-center justify-center">
                   <img src="../image/logo.png" class="w-8 h-8 object-contain animate-pulse">
               </div>
            </div>
            <div class="text-sm font-bold text-slate-800 dark:text-white tracking-widest animate-pulse">MEMUAT DATA...</div>
        </div>
        ...
        <div v-if="isLocked" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-100 dark:bg-slate-900 px-4">
            <div class="w-full max-w-sm bg-white dark:bg-slate-800 rounded-2xl shadow-xl overflow-hidden text-center p-8 animate-fade-in border border-slate-200 dark:border-slate-700">
                ...
                <form @submit.prevent="unlockPage">
                    <input type="text" v-model="username" placeholder="Username" class="w-full text-center text-lg tracking-widest p-3 border rounded-xl bg-slate-50 dark:bg-slate-900 dark:border-slate-600 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none mb-3 transition-all">
                    <input type="password" v-model="accessCode" placeholder="Password" class="w-full text-center text-lg tracking-widest p-3 border rounded-xl bg-slate-50 dark:bg-slate-900 dark:border-slate-600 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none mb-4 transition-all">
                    <button type="submit" class="w-full py-3 bg-sr-blue dark:bg-blue-600 text-white font-bold rounded-xl shadow-lg hover:bg-slate-800 transition-transform active:scale-95">Login</button>
                </form>
            </div>
        </div>
        ...
        <aside class="w-64 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 flex flex-col z-20 flex-shrink-0 h-full shadow-sm transition-colors duration-300">
            ...
            <nav class="flex-1 overflow-y-auto p-3 space-y-1 custom-scrollbar">
                ...
                <a href="package_shipping.php" class="flex items-center px-3 py-2.5 rounded-lg text-sm text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                ...
                <a href="schedule.php" class="flex items-center px-3 py-2.5 rounded-lg text-sm text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                ...
                <a href="penagihan.php" class="flex items-center px-3 py-2.5 rounded-lg text-sm text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
            ...
            <div class="p-3 border-t border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 space-y-1 z-20">
                ...
                <button @click="toggleFullscreen" class="flex items-center px-3 py-2.5 rounded-lg text-sm text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors w-full text-left">
                    <div class="w-6 flex justify-center"><i :class="isFullscreen ? 'bi-fullscreen-exit' : 'bi-arrows-fullscreen'"></i></div>
                ...
                <!-- Logout -->
                <button @click="logout" class="flex items-center px-3 py-2.5 rounded-lg text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors w-full text-left">
                    <div class="w-6 flex justify-center"><i class="bi bi-box-arrow-right"></i></div>
                    <span>Logout</span>
                </button>
            </div>
        </aside>
                        ...
                        <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 flex flex-col shadow-sm overflow-hidden h-full transition-colors">
                            ...
                            <div class="flex-1 overflow-y-auto p-3 space-y-3 custom-scrollbar">
                                ...
                                <div v-for="trip in outboundTrips" :key="trip.id" class="p-3 rounded-lg border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-700 hover:shadow-md transition-all group">
                                    ...
                                    <div class="text-xs font-bold text-slate-700 dark:text-slate-300 mb-1"><i class="bi bi-truck-front text-blue-500"></i> {{ trip.fleet?.name || 'Tanpa Armada' }}</div>
                                    <div class="text-[10px] text-slate-500 dark:text-slate-400 pt-2 border-t border-slate-50 dark:border-slate-600">Driver: <strong>{{ trip.driver?.name || 'Tanpa Supir' }}</strong></div>
                                </div>
                            </div>
                        </div>
                        <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 flex flex-col shadow-sm overflow-hidden h-full transition-colors">
                            ...
                            <div class="flex-1 overflow-y-auto p-3 space-y-3 custom-scrollbar">
                                ...
                                <div v-for="trip in inboundTrips" :key="trip.id" class="p-3 rounded-lg border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-700 hover:shadow-md transition-all group">
                                    ...
                                    <div class="text-xs font-bold text-slate-700 dark:text-slate-300 mb-1"><i class="bi bi-truck-front text-green-500"></i> {{ trip.fleet?.name || 'Tanpa Armada' }}</div>
                                    <div class="text-[10px] text-slate-500 dark:text-slate-400 pt-2 border-t border-slate-50 dark:border-slate-600">Driver: <strong>{{ trip.driver?.name || 'Tanpa Supir' }}</strong></div>
                                </div>
                            </div>
                        </div>
                        <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 flex flex-col shadow-sm overflow-hidden h-full transition-colors">
                            ...
                            <div class="flex-1 overflow-y-auto p-3 space-y-2 custom-scrollbar">
                                <div v-for="f in fleet" :key="f.id" class="flex justify-between items-center p-2 rounded border border-slate-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                                    <div class="flex items-center gap-3"><div class="w-8 h-8 rounded bg-slate-100 dark:bg-slate-600 flex items-center justify-center text-slate-400 dark:text-slate-300"><i :class="f.icon || 'bi-car-front-fill'"></i></div><div><div class="text-xs font-bold text-slate-800 dark:text-slate-200">{{ f.name || 'Tanpa Nama' }}</div><div class="text-[10px] text-slate-400">{{ f.plate || '-' }}</div></div></div>
                        ...
                        <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col h-[calc(100vh-140px)]">
                            ...
                            <div class="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                                ...
                                <div v-for="f in fleet" :key="f.id" class="p-3 rounded-lg border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-700/50 hover:shadow-md transition-all group relative">
                                    <div class="flex justify-between items-start">
                                        <div class="flex items-center gap-3">
                                            <div class="w-10 h-10 rounded-lg bg-slate-100 dark:bg-slate-600 flex items-center justify-center text-slate-500 dark:text-slate-300 text-lg"><i :class="f.icon || 'bi-car-front-fill'"></i></div>
                                            <div>
                                                <div class="font-bold text-slate-800 dark:text-white text-sm">{{ f.name || 'Tanpa Nama' }}</div>
                                                <div class="text-xs text-slate-500 dark:text-slate-400 font-mono">{{ f.plate || '-' }} • {{ f.capacity || 0 }} Seat</div>
                                            </div>
                                        </div>
                                ...
                                <div v-for="d in drivers" :key="d.id" class="p-3 rounded-lg border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-700/50 hover:shadow-md transition-all group relative">
                                    <div class="flex justify-between items-start">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div class="font-bold text-slate-800 dark:text-white text-sm">{{ d.name || 'Tanpa Nama' }}</div>
                    ...
                    <div class="bg-white p-6 rounded-2xl text-center max-w-lg w-full shadow-2xl animate-fade-in">
                        ...
                        <div v-if="(validationData.paymentMethod === 'Transfer' || validationData.paymentMethod === 'DP') && validationData.paymentProof" class="mb-6">
                            <div class="bg-slate-100 p-2 rounded-xl mb-2 border border-slate-200">
                                <img :src="validationData.paymentProof.includes('/') ? validationData.paymentProof : 'transfer/' + validationData.paymentProof" class="w-full h-48 object-cover rounded-lg cursor-pointer hover:opacity-90 transition-opacity" @click="window.open(validationData.paymentProof.includes('/') ? validationData.paymentProof : 'transfer/' + validationData.paymentProof, '_blank')">
                            </div>
                            <a :href="validationData.paymentProof.includes('/') ? validationData.paymentProof : 'transfer/' + validationData.paymentProof" target="_blank" class="text-xs font-bold text-blue-600 hover:underline"><i class="bi bi-box-arrow-up-right mr-1"></i> Lihat Ukuran Penuh</a>
                        </div>
