    ...
    <div id="app" class="flex h-full w-full" v-cloak>
        ...
        <aside class="w-64 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 flex flex-col z-20 flex-shrink-0 h-full shadow-sm">
            ...
            <nav class="flex-1 overflow-y-auto p-3 space-y-1">
                ...
                <a href="#" @click.prevent="setRouteFilter('Padang', 'Laporan Padang')" :class="routeFilter === 'Padang' ? 'bg-blue-50 dark:bg-slate-700 text-blue-700 dark:text-blue-300 font-bold' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700'" class="flex items-center px-3 py-2.5 rounded-lg text-sm transition-colors mb-1">
                    <i class="bi bi-file-earmark-text-fill w-6"></i> Laporan Padang
                </a>
                <a href="#" @click.prevent="setRouteFilter('Bukittinggi', 'Laporan Bukittinggi')" :class="routeFilter === 'Bukittinggi' ? 'bg-blue-50 dark:bg-slate-700 text-blue-700 dark:text-blue-300 font-bold' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700'" class="flex items-center px-3 py-2.5 rounded-lg text-sm transition-colors mb-1">
                    <i class="bi bi-file-earmark-text-fill w-6"></i> Laporan Bukittinggi
                </a>
                <a href="#" @click.prevent="setRouteFilter('Payakumbuh', 'Laporan Payakumbuh')" :class="routeFilter === 'Payakumbuh' ? 'bg-blue-50 dark:bg-slate-700 text-blue-700 dark:text-blue-300 font-bold' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700'" class="flex items-center px-3 py-2.5 rounded-lg text-sm transition-colors mb-1">
                    <i class="bi bi-file-earmark-text-fill w-6"></i> Laporan Payakumbuh
                </a>
        ...
        <main class="flex-1 flex flex-col min-w-0 bg-slate-50 dark:bg-slate-900 relative h-full overflow-hidden">
            <header class="h-16 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between px-6 shadow-sm z-10">
                <h2 class="text-lg font-bold text-slate-800 dark:text-white">{{ currentReportTitle }}</h2>
            ...
            <div v-if="viewMode === 'stats'" class="flex-1 overflow-y-auto p-6 custom-scrollbar">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
                    <div class="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm">
                        ...
                        <div class="text-xl font-extrabold text-slate-800 dark:text-white">{{ formatRupiah(totalRevenue) }}</div>
                    </div>
                    <div class="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm">
                        <div class="text-xs font-bold text-slate-400 uppercase mb-1">Tiket Terjual</div>
                        <div class="text-xl font-extrabold text-slate-800 dark:text-white">{{ totalPax }} <span class="text-sm font-medium text-slate-400">Kursi</span></div>
                    </div>
                            ...
                            <table class="w-full text-sm text-left">
                                <thead class="bg-slate-50 dark:bg-slate-700 text-slate-500 dark:text-slate-300 font-bold uppercase text-xs">
                                    <tr>
                                        ...
                                        <th class="px-6 py-4 text-red-600">Refund (Potongan)</th>
                                ...
                                <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                                    <tr v-for="(label, index) in reversedLabels" :key="index" @click="openDetail(label)" class="group hover:bg-blue-50 dark:hover:bg-slate-700/50 transition-colors relative cursor-pointer">
                                        <td class="px-6 py-4 font-bold text-slate-700 dark:text-slate-200 relative">
                                            {{ formatDate(label) }}

                                        </td>
                                        <td class="px-6 py-4 font-mono text-slate-800 dark:text-white font-bold">
                                            {{ formatRupiah(reversedRevenue[index]) }}
                                            <div v-if="reversedRefundRevenue[index] > 0" class="text-[10px] text-green-600">+ {{ formatRupiah(reversedRefundRevenue[index]) }} (Batal)</div>
                                        </td>
        ...
        createApp({
            data() {
                return {
                    ...
                    // isRefundEditable: false // REMOVED
                }
            },
            computed: {
