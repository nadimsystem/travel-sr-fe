        ...
        <main class="flex-1 flex flex-col min-w-0 bg-slate-50 dark:bg-slate-900 relative h-full overflow-hidden transition-colors duration-300">
            ...
            <div class="flex-1 relative overflow-hidden w-full">
                ...
                <div v-if="view === 'assets'" class="absolute inset-0 p-6 custom-scrollbar space-y-6">
                    <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-4"><h3 class="font-bold text-slate-800 dark:text-white mb-4">Armada</h3><div v-for="f in fleet" :key="f.id" class="flex justify-between items-center p-2 border-b border-slate-100 dark:border-slate-700 last:border-0"><div class="text-sm font-bold text-slate-800 dark:text-slate-200">{{ f.name }}</div><div class="text-xs bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded text-slate-600 dark:text-slate-300">{{ f.status }}</div></div></div>
                </div>
