...
createApp({
    data() {
        return {
            ...
            drivers: [
                ...
                { id: 107, name: 'Agus Setiawan', licenseType: 'B1 Umum', phone: '081234567896', status: 'Tiba' },
            ...
            trips: [
                { id: 1, fleetId: 1, driverId: 101, type: 'Rute', origin: 'Padang', destination: 'Bukittinggi', departureTime: this.getTodayISO(8, 0), arrivalTime: this.getTodayISO(10, 30), pax: '7/7', status: 'Berangkat' },
                { id: 2, fleetId: 14, driverId: 102, type: 'Rental', notes: 'Dinas Pariwisata - Tour de Singkarak', departureTime: this.getTodayISO(7, 30), arrivalTime: this.getTodayISO(18, 0), pax: 'Carter', status: 'Rental' },
                { id: 3, fleetId: 3, driverId: 103, type: 'Rute', origin: 'Padang', destination: 'Payakumbuh', departureTime: this.getTodayISO(10, 0), arrivalTime: this.getTodayISO(13, 0), pax: '5/7', status: 'Boarding' },
                { id: 4, fleetId: 25, driverId: 104, type: 'Rental', notes: 'Rombongan Kemenkes RI (3 Hari)', departureTime: this.getTodayISO(9, 0), arrivalTime: this.getFutureISO(2, 17, 0), pax: 'Carter', status: 'Rental' },
                { id: 5, fleetId: 26, driverId: 110, type: 'Rute', origin: 'Bukittinggi', destination: 'Padang', departureTime: this.getTodayISO(14, 0), arrivalTime: this.getTodayISO(16, 30), pax: '40/45', status: 'Berangkat' },
                { id: 6, fleetId: 4, driverId: 106, type: 'Rute', origin: 'Padang', destination: 'Solok', departureTime: this.getTodayISO(16, 0), arrivalTime: this.getTodayISO(18, 0), pax: 'Full', status: 'On Time' },
                { id: 7, fleetId: 2, driverId: 108, type: 'Rute', origin: 'Painan', destination: 'Padang', departureTime: this.getYesterdayISO(18, 0), arrivalTime: this.getTodayISO(8, 30), pax: '6/7', status: 'Tiba' },
                { id: 8, fleetId: 15, driverId: 107, type: 'Rental', notes: "Trip Keluarga Bpk. Ahmad", departureTime: this.getYesterdayISO(9, 0), arrivalTime: this.getYesterdayISO(20, 0), pax: 'Carter', status: 'Tiba' },
            ],
            locations: [{ name: 'Padang', code: 'PDG' }, { name: 'Bukittinggi', code: 'BKT' }, { name: 'Payakumbuh', code: 'PYH' }, { name: 'Solok', code: 'SLK' }, { name: 'Pariaman', code: 'PRM' }, { name: 'Painan', code: 'PNN' }, { name: 'Sawahlunto', code: 'SWL' }, { name: 'Batusangkar', code: 'BTS' }],
    ...
    computed: {
        currentViewTitle() { const titles = { dashboard: 'Dashboard Operasional', inventaris: 'Manajemen Inventaris Armada', drivers: 'Manajemen Supir', history: 'Riwayat Perjalanan', display: 'Layar Informasi Publik', admin: 'Pengaturan Admin' }; return titles[this.view] || 'Sutan Raya'; },
        tripsWithFleet() {
            return this.trips.map(trip => {
                ...
                const durationDays = this.calculateDurationInDays(trip.departureTime, trip.arrivalTime);
                ...
                return { ...trip, fleet, driver, originCode: trip.type === 'Rute' ? (originInfo ? originInfo.code : 'N/A') : trip.notes, destinationCode: trip.type === 'Rute' ? (destinationInfo ? destinationInfo.code : 'N/A') : null, tripCost, tripRevenue, tripProfit }
            });
        },
        activeTrips() { return this.tripsWithFleet.filter(t => t.status !== 'Tiba' && t.status !== 'Batal'); },
        departures() { return this.activeTrips.filter(t => t.origin === 'Padang').sort((a, b) => new Date(a.departureTime) - new Date(b.departureTime)); },
        arrivals() { return this.activeTrips.filter(t => t.destination === 'Padang').sort((a, b) => new Date(a.arrivalTime) - new Date(b.arrivalTime)); },
        rental() { return this.activeTrips.filter(t => t.type === 'Rental').map(t => ({ ...t, totalDurationFormatted: this.calculateTotalDuration(t.departureTime, t.arrivalTime) })); },
        ...
        selectedDriverNameForWizard() { if (!this.wizard.trip || !this.wizard.trip.driverId) return ''; const driver = this.drivers.find(d => d.id === this.wizard.trip.driverId); return driver ? driver.name : ''; },
        availableFleetForTrip() { let fleet = this.fleet.filter(f => f.status === 'Tersedia'); const trip = this.wizard.trip || this.modal.trip; if (trip && trip.fleetId) { const current = this.fleet.find(f => f.id === trip.fleetId); if (current && !fleet.some(f=>f.id === current.id)) fleet.push(current); } return fleet; },
        availableDriversForTrip() { const trip = this.wizard.trip || this.modal.trip; if (!trip || !trip.fleetId) return []; const selectedFleet = this.fleet.find(f => f.id === trip.fleetId); if (!selectedFleet) return []; let filtered = this.drivers.filter(d => (d.status === 'Standby' || d.id === trip.driverId) && d.licenseType === selectedFleet.requiredLicense); if (this.driverSearchTermInModal) { filtered = filtered.filter(d => d.name.toLowerCase().includes(this.driverSearchTermInModal.toLowerCase())); } return filtered; },
        standbyDrivers() { return this.drivers.filter(d => d.status === 'Standby'); },
    ...
    methods: {
        ...
        formatRupiah(number) { if (isNaN(number)) return 'Rp 0'; return new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(number); },
        ...
        getFutureISO(days, h, m) { const d = new Date(); d.setDate(d.getDate() + days); d.setHours(h, m, 0, 0); return d.toISOString().slice(0, 16); },
        isServiceDue(nextServiceDate) { const nextService = new Date(nextServiceDate); const thirtyDaysFromNow = new Date(this.now); thirtyDaysFromNow.setDate(this.now.getDate() + 30); return nextService < thirtyDaysFromNow; },
        getVehicleStatusClass(status) { return status === 'Tersedia' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'; },
        getDriverStatusClass(status) { const classes = { 'Standby': 'bg-green-100 text-green-800', 'Dalam Perjalanan': 'bg-blue-100 text-blue-800', 'Libur': 'bg-gray-200 text-gray-700' }; return classes[status] || ''; },
        calculateDurationInDays(start, end) { const startDate = new Date(start); const endDate = new Date(end); if (isNaN(startDate) || isNaN(endDate) || endDate < startDate) return 0; const diffTime = endDate - startDate; const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); return diffDays < 1 ? 1 : diffDays; },
        calculateTotalDuration(start, end) { const diff = (new Date(end).getTime() - new Date(start).getTime()) / 1000; if (diff <= 0) return '0 menit'; const days = Math.floor(diff / 86400); const hours = Math.floor((diff % 86400) / 3600); const minutes = Math.floor((diff % 3600) / 60); let result = ''; if (days > 0) result += `${days} hari `; if (hours > 0) result += `${hours} jam `; if (minutes > 0) result += `${minutes} mnt`; return result.trim() || '0 mnt'; },
        calculateElapsedTime(start) { const startDate = new Date(start); if (this.now < startDate) return 'Belum Mulai'; let diff = (this.now.getTime() - startDate.getTime()) / 1000; const hours = Math.floor(diff / 3600); const minutes = Math.floor((diff % 3600) / 60); const seconds = Math.floor(diff % 60); return [hours, minutes, seconds].map(v => v < 10 ? "0" + v : v).join(":"); },
        calculateProgress(trip) { const start = new Date(trip.departureTime); const end = new Date(trip.arrivalTime); if (this.now < start) return 0; if (this.now >= end) return 100; const total = end.getTime() - start.getTime(); if (total <= 0) return 100; const elapsed = this.now.getTime() - start.getTime(); return (elapsed / total) * 100; },
        hideDriverDropdownWithDelay() { setTimeout(() => { this.showDriverDropdown = false; }, 200); },
        selectDriver(driver, context = 'wizard') { let trip = context === 'wizard' ? this.wizard.trip : this.modal.trip; if(trip) trip.driverId = driver.id; this.driverSearchTermInModal = driver.name; this.showDriverDropdown = false; },
        setWizardDate(type) { this.wizard.date.type = type; let newDate; if (type === 'today') { newDate = new Date(); } else if (type === 'tomorrow') { newDate = new Date(); newDate.setDate(newDate.getDate() + 1); } else { return; } this.wizard.date.raw = newDate.toISOString().slice(0, 10); },
        openTripWizard(standbyFleetId = null) { this.driverSearchTermInModal = ''; this.wizard.trip = { id: Date.now(), fleetId: standbyFleetId || '', driverId: null, type: 'Rute', origin: 'Padang', destination: 'Bukittinggi', notes: '', departureTime: '', arrivalTime: '', pax: '', status: 'On Time', }; this.setWizardDate('today'); this.wizard.time = '08:00'; const todayStr = new Date().toISOString().slice(0,10); this.wizard.endDate.raw = todayStr; this.wizard.endTime = '17:00'; this.wizard.step = 1; this.isTripWizardVisible = true; },
        closeTripWizard() { this.isTripWizardVisible = false; },
        saveTripFromWizard() { if (!this.wizard.trip.fleetId || !this.wizard.trip.driverId) { alert('Silakan pilih Armada dan Supir pada Langkah 3.'); this.wizard.step = 3; return; } if (this.wizard.trip.type === 'Rute') { this.wizard.trip.departureTime = `${this.wizard.date.raw}T${this.wizard.time}`; const departureDate = new Date(this.wizard.trip.departureTime); departureDate.setHours(departureDate.getHours() + 2); this.wizard.trip.arrivalTime = departureDate.toISOString().slice(0, 16); } else { this.wizard.trip.departureTime = `${this.wizard.date.raw}T${this.wizard.time}`; this.wizard.trip.arrivalTime = `${this.wizard.endDate.raw}T${this.wizard.endTime}`; } this.trips.push(this.wizard.trip); this.updateDriverStatusOnTripChange(null, this.wizard.trip.driverId, this.wizard.trip.status); this.closeTripWizard(); },
        openTripModal(trip) { this.modal.trip = JSON.parse(JSON.stringify(this.tripsWithFleet.find(t => t.id === trip.id))); this.isTripModalVisible = true; },
        closeTripModal() { this.isTripModalVisible = false; },
        saveTrip() { if (!this.modal.trip) return; const index = this.trips.findIndex(t => t.id === this.modal.trip.id); if (index !== -1) { const { fleet, driver, ...originalTrip } = this.modal.trip; this.trips[index] = { ...this.trips[index], ...originalTrip }; } this.closeTripModal(); },
        updateTripStatus(tripId, newStatus) { const tripIndex = this.trips.findIndex(t => t.id === tripId); if (tripIndex === -1) return; const trip = this.trips[tripIndex]; const oldStatus = trip.status; if (oldStatus === newStatus) return; trip.status = newStatus; this.modal.trip.status = newStatus; if ((newStatus === 'Tiba' || newStatus === 'Batal') && oldStatus !== 'Tiba' && oldStatus !== 'Batal') { const driverIndex = this.drivers.findIndex(d => d.id === trip.driverId); if (driverIndex !== -1) this.drivers[driverIndex].status = 'Standby'; } else if (newStatus !== 'Tiba' && newStatus !== 'Batal' && (oldStatus === 'Tiba' || oldStatus === 'Batal')) { const driverIndex = this.drivers.findIndex(d => d.id === trip.driverId); if (driverIndex !== -1) this.drivers[driverIndex].status = 'Dalam Perjalanan'; } },
        ...
        deleteTrip(tripId) { if (confirm('Apakah Anda yakin ingin menghapus jadwal perjalanan ini?')) { const trip = this.trips.find(t=> t.id === tripId); if(trip && trip.driverId) { const driverIndex = this.drivers.findIndex(d=>d.id === trip.driverId); if(driverIndex !== -1 && this.drivers[driverIndex].status === 'Dalam Perjalanan') this.drivers[driverIndex].status = 'Standby'; } this.trips = this.trips.filter(t => t.id !== tripId); this.closeTripModal(); } },
        openVehicleModal(vehicle) { if (vehicle) { this.vehicleModal.mode = 'edit'; this.vehicleModal.data = JSON.parse(JSON.stringify(vehicle)); } else { this.vehicleModal.mode = 'add'; this.vehicleModal.data = { id: Date.now(), name: '', type: 'Hiace Premio', plate: '', capacity: 7, status: 'Tersedia', icon: 'bi-truck-front-fill', hargaSewa: 1600000, hargaPerOrang: 150000, biayaOperasional: 600000, requiredLicense: 'A Umum', lastService: this.getTodayISO(0,0).slice(0,10), nextService: this.getFutureISO(180,0,0).slice(0,10) }; } this.isVehicleModalVisible = true; },
        closeVehicleModal() { this.isVehicleModalVisible = false; },
        saveVehicle() { if (this.vehicleModal.data.type.toLowerCase().includes('bus')) { this.vehicleModal.data.icon = 'bi-bus-front-fill'; } else { this.vehicleModal.data.icon = 'bi-truck-front-fill'; } if (this.modal.mode === 'add') { this.fleet.push(this.vehicleModal.data); } else { const index = this.fleet.findIndex(f => f.id === this.vehicleModal.data.id); if (index !== -1) { this.fleet[index] = this.vehicleModal.data; } } this.closeVehicleModal(); },
    ...
    watch: {
        'wizard.date.raw'(newDate) {
            // Automatically set end date to be same as start date for new rentals
