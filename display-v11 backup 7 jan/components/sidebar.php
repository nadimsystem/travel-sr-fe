<aside class="w-64 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 flex flex-col z-20 flex-shrink-0 h-full shadow-sm transition-colors duration-300">
    ...
    <nav class="flex-1 overflow-y-auto p-3 space-y-1 custom-scrollbar">
        ...
        <a href="booking_bus.php" class="flex items-center px-3 py-2.5 rounded-lg text-sm transition-colors <?= $currentPage == 'booking_bus' ? 'bg-blue-50 dark:bg-slate-700 text-blue-700 dark:text-blue-300 font-bold' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700' ?>">
            ...
            <a href="paket.php" class="flex items-center gap-3 px-4 py-3 rounded-2xl transition-all duration-300 group <?php echo $currentPage == 'paket' ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' : 'text-slate-500 hover:bg-slate-50 hover:text-blue-600'; ?>">
                <i class="bi bi-box-seam text-lg"></i>
                <span class="font-bold text-sm">Kirim Paket</span>
            </a>
        ...
        <a href="billing_management.php" class="flex items-center px-3 py-2.5 rounded-lg text-sm transition-colors <?= $currentPage == 'billing_management' ? 'bg-blue-50 dark:bg-slate-700 text-blue-700 dark:text-blue-300 font-bold' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700' ?>">
            <i class="bi bi-cash-stack w-6"></i> Penagihan
        </a>
